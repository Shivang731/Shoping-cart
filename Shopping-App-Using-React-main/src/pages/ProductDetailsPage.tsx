...
export default function ProductDetailsPage() {
  ...
  if (error || !product) {
    return <div className="min-h-screen p-6 text-center text-red-500">{error || 'Product not found.'}</div>
  }
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid gap-6 lg:grid-cols-2">
          <article className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            >
              {[1, 2, 3].map((slide) => (
                <SwiperSlide key={slide} className="flex items-center justify-center bg-gray-100">
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 flex items-center gap-2 text-sm text-yellow-500">
              {Array.from({ length: 5 }, (_, i) => (
                <span key={i}>{product.rating.rate >= i + 1 ? '★' : '☆'}</span>
              ))}
              <span className="text-gray-500">({product.rating.count})</span>
            </div>
            <p className="mt-4 text-xl font-bold text-indigo-600">${product.price.toFixed(2)}</p>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <button
                ...
                className={`flex-1 rounded-xl border border-gray-200 px-4 py-2 text-sm font-medium transition ${wishlistActive ? 'bg-red-50 text-red-600' : 'bg-white text-gray-700 hover:bg-gray-100'}`}
              >
